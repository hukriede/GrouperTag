...
Modified by Nicholas Hukriede of Mote Marine Laboratories to include a Blue Robotics Bar30 sensor.
Pressure, temperature, depth readings are read from the sensor and output in line with the standard values from
the SparkFun board. Pressure in mbar, temperature in degrees C, and depth in meters. 
******************************************************************************/
...
// Creates and logs the IMU and Bar30 sensor data to the serial monitor
void logData(void) {
  ...
  if (enableSensor) {
    ...
    imuLog += String(sensor.pressure()) + ", ";    // Output in mbar
    imuLog += String(sensor.temperature()) + ", "; // Output in degrees Celsius
    imuLog += String(sensor.depth()) + ", ";       // Output in meters
  }
...
// Initializes the hardware for future set ups
...
// Initializes the pressure/temperature sensor to be read
// Sets density initial values and model number
// Returns whether the sensor has been successfully initialized
bool initSens() {
  Wire.begin();
  if (!sensor.init()) {
    return false;
...
// Initializes the IMU board and sets up necessary variables
// Returns whether the IMU has been successfully initialized
bool initIMU(void) {
...
// Initializes SD card for reading/writing
// Returns whether the SD card has been successfully initialized
bool initSD(void) {
...
// Returns whether the log file opened properly and if the string was added
...
#endif
    break;
  case ENABLE_SENSOR: //Enable/disable sensor output (pressure, temp, depth)
